...
import org.emu.search.config.KafkaProperties;
...
class SearchingServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        SearchingServiceKafkaResource kafkaResource = new SearchingServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/searching-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/searching-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
